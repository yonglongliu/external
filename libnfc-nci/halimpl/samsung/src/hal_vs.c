...
bool hal_vs_check_rfreg_update(tNFC_HAL_VS_INFO *vs, tNFC_NCI_PKT *pkt)
{
    ...
    {
        sprintf(rfreg_date_version_buffer, "%d/%d/%d/%d.%d.%d",
                (vs->rfreg_img_version[0] >> 4) + 14, vs->rfreg_img_version[0] & 0xF, vs->rfreg_img_version[1],
                vs->rfreg_img_version[2], vs->rfreg_img_version[3], vs->rfreg_img_version[4] );
    ...
    sprintf(rfreg_csc_buffer, "%c%c%c", vs->rfreg_version[5], vs->rfreg_version[6], vs->rfreg_version[7]);
