...
/* [201603] Single SKU */
        ...
        select_image_set_fw_version(NCI_MF_INFO(pkt), NCI_MF_INFO_SIZE - 1);    // User only 3 bytes

        // Get RF version and CSC code from the NFC chip
        if(SNFC_N8 <= bl->version[0])
...
/* [201603] Single SKU */
        ...
        {
            if (((SNFC_N8 <= bl->version[0]) && (NCI_OID(pkt) == NCI_PROP_GET_OPTION_META_OID)) ||
                ((SNFC_N8 > bl->version[0]) && (NCI_OID(pkt) == NCI_PROP_GET_RFREG_VER)))
            {
                // Set RF register version
                hal_vs_set_rfreg_version(vs, pkt);
                select_image_set_rf_version(vs->rfreg_version, RFREG_VERSION_SIZE - 2); // User 6 bytes
                if(NCI_OID(pkt) == NCI_PROP_GET_OPTION_META_OID)
                    select_image_set_chip_code(NCI_PAYLOAD(pkt) + 12);
                else if(NCI_OID(pkt) == NCI_PROP_GET_RFREG_VER)
                    select_image_set_chip_code(NCI_PAYLOAD(pkt) + 6);

#ifndef NFC_HAL_DO_NOT_USE_BOOTLOADER
                if (!(nfc_hal_info.flag & HAL_FLAG_FORCE_FW_UPDATE))
                {
                    if (nfc_fw_get_image(fw, nfc_hal_info.cfg.fw_file, bl->version[NFC_HAL_BL_VER_KEYINFO], false))
                    {
...
/* [201603] End */
    ...
    complete:
        ...
        OSI_logd("Complete: CSC: %c%c%c", vs->rfreg_version[5], vs->rfreg_version[6], vs->rfreg_version[7]);
        OSI_logd("Complete: NFC FW Version: %02x.%02x.%02x",
            fw->fw_ver.major, fw->fw_ver.build1, fw->fw_ver.build2);
        OSI_logd("Complete: NFC RF Version: %d/%d/%d/%d.%d.%d",
            (vs->rfreg_version[0] >> 4) + 14, vs->rfreg_version[0] & 0xF, vs->rfreg_version[1],
            vs->rfreg_version[2], vs->rfreg_version[3], vs->rfreg_version[4] );
        if(SNFC_N8 <= bl->version[0])
        {
