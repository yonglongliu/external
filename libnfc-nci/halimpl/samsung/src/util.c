...
{
    static char prefix_buffer[5] = {'\0', };    /* Length of CSC value is 3 + '+' + '\0\' */
...
bool get_is_last_field_overrode(void)
{
    return last_field_override;
}
/* [201603] End */
...
bool find_by_name(FILE *file, char *field)
{
    ...
    if (userPrefix != NULL)
    {
        ...
        {
            OSI_logd("The prefix configuration is exist: %s will be replaced to %s", field, firstField);
...
int get_config_byteArry(char *field, uint8_t *byteArry, size_t arrySize)
{
    ...
    for (i = 0; i < count; i++)
        sprintf((prtb + (i*5)), "0x%02X ", buffer[i]);
